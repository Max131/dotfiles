...
### Set variables
#################
PATH="/usr/local/bin:/usr/local/sbin/:$(ruby -e 'puts Gem.user_dir')/bin":/home/m13/.local/share/gem/ruby/3.0.0/bin:/home/m13/bin:$PATH
...
PS1="[%(!.${PR_RED}%n.$PR_LIGHT_BLUE%n)%(!.${PR_LIGHT_YELLOW}@.$PR_RED@)$PR_NO_COLOR%(!.${PR_LIGHT_RED}%U%m%u.${PR_LIGHT_BLUE}%U%m%u)$PR_NO_COLOR:%(!.${PR_RED}%2c.${PR_BLUE}%2c)$PR_NO_COLOR]%(?..[${PR_LIGHT_RED}%?$PR_NO_COLOR])%(!.${PR_LIGHT_RED}#.${PR_LIGHT_GREEN}$) "
